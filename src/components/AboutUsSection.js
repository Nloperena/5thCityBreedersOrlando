...
function AboutUsSection() {
  ...
  return (
    <div className="p-8 md:p-16 lg:p-24 bg-gradient-to-b from-gray-100 to-gray-200 rounded-xl shadow-lg mt-10 space-y-10 relative overflow-hidden">
      {/* Soft paw pattern in background */}
      ...
      <h2 
        className="text-3xl md:text-5xl font-bold text-blue-700 mb-6 text-center mt-4 underline decoration-wavy" 
        data-aos="fade-down"
      >
        About Us
      </h2>
      ...
      {videos.map((video, index) => (
        ...
        >
          <div className="md:w-1/2 w-full flex flex-col justify-center text-center md:text-left mb-6 md:mb-0 p-4 space-y-4">
            ...
            <p className="text-xl md:text-2xl lg:text-3xl font-semibold text-blue-700 bg-blue-100 px-4 py-2 rounded-lg shadow-sm">
              {content[index].seoPhrase}
            </p>
          </div>

          {/* Video Section with tilt and scale hover */}
          <div 
            className="md:w-1/2 w-full rounded-2xl overflow-hidden shadow-2xl relative transform transition-transform duration-300 hover:scale-105 hover:rotate-1 hover:shadow-3xl"
            style={{ maxHeight: '100%' }}
          >
            <iframe 
      ...
      <div className="flex justify-center space-x-4 mt-10" data-aos="fade-up">
        <button 
